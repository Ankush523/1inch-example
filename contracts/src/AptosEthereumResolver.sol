...
/**
 * @title Enhanced Resolver for Ethereum-Aptos Cross-chain Swaps
 * @dev Extends the base resolver to support bidirectional swaps with Aptos blockchain
 * @custom:security-contact security@1inch.io
 */
contract AptosEthereumResolver is Ownable {
    ...
    error SwapNotFound();
    error InvalidAptosAddress();

    /// @notice Swap direction constants
    uint8 public constant SWAP_ETH_TO_APTOS = 0;
    uint8 public constant SWAP_APTOS_TO_ETH = 1;

    /// @notice Cross-chain swap metadata
    struct CrossChainSwap {
        bytes32 swapId;                    // Unique swap identifier
        uint8 direction;                   // 0 = ETH->Aptos, 1 = Aptos->ETH
        bytes32 orderHash;                 // Ethereum order hash
        address ethEscrowAddress;          // Ethereum escrow contract address
        string aptosEscrowId;              // Aptos escrow ID (as string)
        address makerEthAddress;           // Ethereum maker address
        address takerEthAddress;           // Ethereum taker address
        string makerAptosAddress;          // Aptos maker address
        string takerAptosAddress;          // Aptos taker address
        uint256 amountEth;                 // Amount on Ethereum side
        uint256 amountAptos;               // Amount on Aptos side (in Aptos units)
    ...
    event AptosSwapCompleted(
        bytes32 indexed swapId,
        bytes32 secret,
        address indexed recipient
    );

    event AptosSwapCancelled(
        bytes32 indexed swapId,
    ...
    /**
     * @notice Initiate ETH to Aptos swap by deploying source escrow
     ...
     * @param makerAptosAddress Maker's Aptos address
     * @param takerAptosAddress Taker's Aptos address
     * @param amountAptos Amount on Aptos side
     */
    function initiateEthToAptosSwap(
        bytes32 swapId,
        ...
        bytes calldata args,
        string calldata makerAptosAddress,
        string calldata takerAptosAddress,
        uint256 amountAptos
    ) external payable onlyOwner {
        require(!swapExists[swapId], "Swap already exists");
        require(bytes(makerAptosAddress).length > 0, "Invalid maker Aptos address");
        require(bytes(takerAptosAddress).length > 0, "Invalid taker Aptos address");
        ...
        CrossChainSwap memory swap = CrossChainSwap({
            ...
            aptosEscrowId: "", // Will be set when Aptos escrow is created
            makerEthAddress: immutables.maker.get(),
            takerEthAddress: immutables.taker.get(),
            makerAptosAddress: makerAptosAddress,
            takerAptosAddress: takerAptosAddress,
            amountEth: amount,
            amountAptos: amountAptos,
    ...
    /**
     * @notice Initiate Aptos to ETH swap by deploying destination escrow
     * @param swapId Unique identifier for the swap
     ...
     * @param amountAptos Amount on Aptos side
     */
    function initiateAptosToEthSwap(
        bytes32 swapId,
    ...
    function completeSwap(
        ...
        emit AptosSwapCompleted(swapId, secret, msg.sender);
    }
    ...
    function cancelSwap(
        ...
        emit AptosSwapCancelled(swapId, msg.sender);
    }

    /**
     * @notice Update Aptos escrow ID for ETH to Aptos swaps
     * @param swapId The swap identifier
     * @param aptosEscrowId The Aptos escrow ID
     */
    function updateAptosEscrowId(bytes32 swapId, string calldata aptosEscrowId) external onlyOwner {
        require(swapExists[swapId], "Swap not found");
        
        CrossChainSwap storage swap = swaps[swapId];
        require(swap.direction == SWAP_ETH_TO_APTOS, "Invalid swap direction");
        
        swap.aptosEscrowId = aptosEscrowId;
    }
